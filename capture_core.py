...
class Core():
    ...
    temp_file = None
    # 计数器
    counter = {"ipv4": 0, "ipv6": 0, "tcp": 0, "udp": 0, "icmp": 0, "arp": 0, "Modbus/TCP": 0, "DNP3.0": 0, "S7COMM": 0}
            ...
            if not self.pause_flag and packet.name == "Ethernet":
                ...
                if ether_type != "ARP":
                    ...
                    if sport and dport:
                        ...
                        elif sport == 20000 or dport == 20000:
                            https = packet.payload.payload.payload.__bytes__().hex()
                            if https[0:4] == '0564':
                                protocol = "DNP3.0"
                                self.counter["DNP3.0"] += 1
                        elif sport == 4185 or dport == 4185 or sport == 4205 or dport == 4205 or sport == 4305 or dport == 4305 \
                                or sport == 4173 or dport == 4173 or sport == 4258\
                                or dport == 4258 or sport == 4272 or dport == 4272:
                            https = packet.payload.payload.payload.__bytes__().hex()
                            if len(https) >= 16 and https[14:16] == '32':
                                protocol = "S7COMM"
                                self.counter["S7COMM"] += 1
                        elif sport in ports:
                            protocol = ports[sport] + version_add
                            if sport ==502:
                                self.counter["Modbus/TCP"] += 1
                        elif dport in ports:
        ...
        try:
            ...
            # 第五层: 应用层
                ...
                while len(https) >= 0:
                    ...
                    if https[4:8] == '0000':
                        protocol = "Modbus/TCP"
                        first_return.append("%s" % protocol)
                        transactionid = int(https[0:4],base = 16)
                        procotolid = int(https[4:8],base = 16)
                        length = int(https[8:12],base = 16)
                        unitid = int(https[12:14],base = 16)
                        fc = int(https[14:16],base = 16)
                        function = functioncode[fc]
                        next_layer.append("Transaction Identifier: %s" % transactionid)
                        next_layer.append("Protocol Identifier: %s" % procotolid)
                        next_layer.append("Length: %s" % length)
                        next_layer.append("Unit Identifier: %s" % unitid)

                        second_return.append(next_layer.copy())
                        next_layer.clear()
                        first_return.append("Modbus")
                        next_layer.append("Function Code: %s" % fc)
                        next_layer.append("Function: %s" % function)
                        #读取线圈/输入状态
                        if fc == 1 or fc == 2:
                            if length == 6:
                                refer = int(https[16:20],base = 16)
                                bit = int(https[20:24],base = 16)
                                next_layer.append("Reference Number: %s" % refer)
                                next_layer.append("Bit Count: %s" % bit)
                                next_layer.append("[Request Frame]")
                            if length == 16:
                                byte = int(https[16:18],base = 16)
                                data = https[18:total_length]
                                next_layer.append("Byte Count: %s" % byte)
                                next_layer.append("Data: %s" % data)
                                next_layer.append("[Response Frame]")
                        #读取保持/输入寄存器
                        if fc == 3 or fc == 4:
                            if length == 6:
                                refer = int(https[16:20],base = 16)
                                word = int(https[20:24],base = 16)
                                next_layer.append("Reference Number: %s" % refer)
                                next_layer.append("Word Count: %s" % word)
                                next_layer.append("[Request Frame]")
                            if length == 203:
                                byte = int(https[16:18],base = 16)
                                next_layer.append("Byte Count: %s" % byte)
                                for i in range(0,99):
                                    start = 18+4*i
                                    stop = 22+4*i
                                    temp = int(https[start:stop],base = 16)
                                    next_layer.append("Register: %d (UINT16): %s" % (i, temp))
                                next_layer.append("[Response Frame]")
                        #读写多个寄存器
                        if fc == 23:
                            if length == 31:
                                readrefer = int(https[16:20],base = 16)
                                readwordcount = int(https[20:24],base = 16)
                                writerefer = int(https[24:28],base = 16)
                                writewordcount = int(https[28:32],base = 16)
                                byte = int(https[32:34],base = 16)
                                data = https[34:total_length]
                                next_layer.append("Read Reference Number: %s" % readrefer)
                                next_layer.append("Read Word Count: %s" % readwordcount)
                                next_layer.append("Write Reference Number: %s" % writerefer)
                                next_layer.append("Write Word Count: %s" % writewordcount)
                                next_layer.append("Byte Count: %s" % byte)
                                next_layer.append("Data: %s" % data)
                                next_layer.append("[Request Frame]")
                            if length == 13:
                        ...
                        if fc == 5:
                            ...
                            next_layer.append("Reference Number: %s" % refer)
                            next_layer.append("Data: %s" % data)
                            next_layer.append("Padding: 0x%s" % padding)
                        #预置单寄存器
                        if fc == 6:
                            refer = int(https[16:20],base = 16)
                            data = https[20:24]
                        ...
                        if fc == 22:
                            ...
                            ormask = https[24:28]
                            next_layer.append("Reference Number: %s" % refer)
                            next_layer.append("AND mask: 0x%s" % andmask)
                            next_layer.append("OR mask: 0x%s" % ormask)
                        #报警
                        if fc == 5 or fc == 6 or fc == 15 or fc == 16 or fc == 21 or fc == 23:
                            second_return.append(next_layer.copy())
                            next_layer.clear()
                            first_return.append("Warning！")
                            next_layer.append("警告！该数据包请求进行写操作！请注意是否有非法请求！")

                    # DNP3
                    if https[0:4] == '0564' and total_length >= 20:
                        protocol = "Distributed Network Protocol 3.0 :"
                        first_return.append("%s" % protocol)
                        next_layer.append(" ")
                        #Data Link Layer
                        second_return.append(next_layer.copy())
                        next_layer.clear()
                        first_return.append("Data Link Layer" )
                        next_layer.append("Start Bytes: 0x%s" % https[0:4])
                        length = int(https[4:6],base = 16)
                        dst = int(https[8:10], base=16)
                        src = int(https[12:14], base=16)
                        checksum1 = https[18:20]
                        checksum2 = https[16:18]
                        next_layer.append("Length: %s" % length)
                        next_layer.append("Control: 0x%s " % https[6:8])
                        next_layer.append("Destination: %s" % dst)
                        next_layer.append("Source: %s" % src)
                        next_layer.append("Data Link Header checksum: 0x%s%s" % (checksum1,checksum2))
                        #Transport Control
                        if total_length >= 22:

                            second_return.append(next_layer.copy())
                            next_layer.clear()
                            first_return.append("Transport Control: 0x%s" % https[20:22])
                            next_layer.append(" ")
                            #Data Chunks
                            if total_length >= 30:
                                # second_return.append(next_layer.copy())
                                # next_layer.clear()
                                # first_return.append("Data Chunks:")
                                # next_layer.append("Data Chunk1: %s" % https[20:52])
                                # next_layer.append("Data Chunk2: %s" % https[56:64])
                                #Application Layer
                                second_return.append(next_layer.copy())
                                next_layer.clear()
                                first_return.append("Application Layer:")
                                next_layer.append("Application Control: 0x%s" % https[22:24])
                                next_layer.append("Function Code: 0x%s" % https[24:26])
                                next_layer.append("Request Data: %s" % https[26:total_length-4])

                    #S7COMM
                    if total_length >= 34 and https[14:16] == '32':
                        protocol = "S7 Communication"
                        #TPKT
                        version = int(https[0:2],base = 16)
                        reserved = int(https[2:4], base=16)
                        length = int(https[4:8], base=16)
                        first_return.append("TPKT,version:%s ,Length:%s" %(version,length))
                        next_layer.append("Version: %s" % version)
                        next_layer.append("Reserved: %s" % reserved)
                        next_layer.append("Length: %s" % length)
                        second_return.append(next_layer.copy())
                        next_layer.clear()
                        #ISO 8073/X.224 COTP Connection-Oeiented Transport Protocol
                        first_return.append("ISO 8073/X.224 COTP Connection-Oeiented Transport Protocol")
                        length = int(https[8:10], base=16)
                        next_layer.append("Length: %s" % length)
                        next_layer.append("PDU Type: DT Data (0x%s)" % https[10:12])
                        next_layer.append("[Destination reference:0x%s]" % https[12:14])
                        second_return.append(next_layer.copy())
                        next_layer.clear()
                        #S7COMM
                        first_return.append("%s" % protocol)
                        rosctr = int(https[16:18], base=16)
                        unit = int(https[22:26], base=16)
                        pdu_length = int(https[26:30], base=16)
                        data_length = int(https[30:34], base=16)
                        next_layer.append("Header: ")
                        next_layer.append("    Protocol Id: 0x%s" % https[14:16])
                        next_layer.append("    ROSCTR: (%s)" % rosctr)
                        next_layer.append("    Redundancy Identification (Reserved): 0x%s" % https[18:22])
                        next_layer.append("    Protocol Data Unit Reference: %s" % unit)
                        next_layer.append("    Parameter length: %s" % pdu_length)
                        next_layer.append("    Data length: %s" % data_length)

                        temp = 34 + pdu_length + pdu_length
                        next_layer.append("Parameter: %s" %https[34:temp])

                        if data_length > 0:
                            temp2 = 34 + pdu_length + pdu_length + data_length +data_length
                            next_layer.append("Data: %s" % https[temp:temp2])
                        '''
                        calling = int(https[38:42], base=16)
                        called = int(https[42:46], base=16)
                        pdulen = int(https[46:50], base=16)
                        next_layer.append("    Function: 0x%s" % https[34:36])
                        next_layer.append("    Reserved: 0x%s" % https[36:38])
                        next_layer.append("    Max AmQ calling: %s" % calling)
                        next_layer.append("    Max AmQ called" % called)
                        next_layer.append("    PDU length: %s" % pdulen)
                        '''

                    # TLS
                    if https[2:4] == '03' and https[0:2] in content_type and https[4:6] in version:
                        protocol = version[https[4:6]]
                        ...
                        if length > 0:
                            ...
                            next_layer.append("Data: %s" % https[10:this_layer_len])
    ...
    def get_transport_count(self):
        ...
        获取传输层数据包的数量
        """
        the_keys = ['tcp', 'udp', 'icmp', 'arp','Modbus/TCP','DNP3.0','S7COMM']
